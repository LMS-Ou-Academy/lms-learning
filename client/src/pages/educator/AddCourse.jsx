...
const AddCourse = () => {
  ...
  const [currentChapterId, setCurrentChaptersId] = useState(null)
  ...
  return (
    <div className='flex flex-col items-start justify-between h-screen p-4 pb-0 overflow-scroll md:p-8 md:pb-0'>
      <form className='flex flex-col gap-4 max-w-md w-full text-gray-500' onSubmit={handleSubmit}>
        <div className="flex flex-col gap-1">
          <p>Course Title</p>
          <input type="text" onChange={e => setCourseTitle(e.target.value)} value={courseTitle} className='px-3 py-2 border border-gray-500 rounded outline-none md:py-2.5' required placeholder='Type here' />
        </div>
        <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-1">
          <p>Discount %</p>
          <input type="number" className='py-2 outline-none md:py-2.5 w-28 px-3 rounded border border-gray-500' required onChange={e => setDiscount(e.target.value)} value={discount} placeholder='0' min={0} max={100} />
        </div>
        ...
        <button type="button" className='w-max my-4 bg-black text-white px-8 py-2.5 rounded'>ADD</button>
      </form>
    </div>
  );
};
